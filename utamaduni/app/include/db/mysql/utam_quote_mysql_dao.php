...
include_once (dirname (__FILE__) . '/../dao/utam_quote_dao.php');
...
/**
 * Class that operate on table 'utam_quote'. Database MySQL.
 *
 * @author: Román Ginés Martínez Ferrández <romangines@riseup.net>
 */
class utam_quote_mysql_dao implements utam_quote_dao
{
	/**
	 * Get domain object by primary key.
	 *
	 * @param String $id primary key.
	 * @return utam_quote_mysql.
	 */
	...
	public function query_all_order_by ($order_col)
	 {
		$sql = 'SELECT * FROM utam_quote ORDER BY ' . $order_col;
		$query = new sql_query ($sql);
		return $this -> get_list ($query);
	}
	 ...
	 *
	 * @param String $id utam_quote primary key
	 */
	public function delete ($id)
	{
		$sql = 'DELETE FROM utam_quote WHERE id = ?';
		$query = new sql_query ($sql);
		$query -> set_number ($id);
	...
	{
		$sql = 'SELECT * FROM utam_quote WHERE id = ?';
		$query = new sql_query ($sql);
		$query -> set ($value);
		return $this -> get_list ($query);
	}
	...
	public function query_by_quote ($value)
	{
		$sql = 'SELECT * FROM utam_quote WHERE quote = ?';
		$query = new sql_query ($sql);
		$query -> set ($value);
		return $this -> get_list ($query);
	}

	public function delete_by_quote ($value)
	{
		$sql = 'DELETE FROM utam_quote WHERE quote = ?';
		$query = new sql_query ($sql);
		$query -> set ($value);
		return $this -> execute_update ($query);
	}
	...
	{
		$sql = 'SELECT * FROM utam_quote WHERE author = ?';
		$query = new sql_query ($sql);
		$query -> set ($value);
		return $this -> get_list ($query);
	}
	...
	{
		$sql = 'DELETE FROM utam_quote WHERE author = ?';
		$query = new sql_query ($sql);
		$query -> set ($value);
		return $this -> execute_update ($query);
	}
	 ...
	 *
	 * @param utam_quote_mysql utam_quote.
	 */
	public function insert ($utam_quote)
	{
		$sql = 'INSERT INTO utam_quote (id, quote, author) VALUES (?, ?, ?)';
		$query = new sql_query ($sql);

		$query -> set ($utam_quote -> id);
		$query -> set ($utam_quote -> quote);
		$query -> set ($utam_quote -> author);
	 ...
	 * @param utam_quote_mysql utam_quote
	 */
	public function update ($utam_quote)
	{
		$sql = 'UPDATE utam_quote SET id = ?, quote = ?, author = ? WHERE id = ?';
		$query = new sql_query ($sql);
		...
		$query -> set_number ($utam_quote -> id);
	...
	{
		$utam_quote = new utam_quote ();

		$utam_quote -> id = $row['id'];
		$utam_quote -> quote = $row['quote'];
		$utam_quote -> author = $row['author'];
