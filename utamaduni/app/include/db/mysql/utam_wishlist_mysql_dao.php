...
include_once (dirname (__FILE__) . '/../dao/utam_wishlist_dao.php');
...
/**
 * Class that operate on table 'utam_wishlist'. Database MySQL.
 *
 * @author: Román Ginés Martínez Ferrández <romangines@riseup.net>
 */
class utam_wishlist_mysql_dao implements utam_wishlist_dao
{
	/**
	 * Get domain object by primary key.
	 *
	 * @param String $id primary key.
	 * @return utam_wishlist_mysql.
	 */
	...
	public function query_all_order_by ($order_col)
	 {
		$sql = 'SELECT * FROM utam_wishlist ORDER BY ' . $order_col;
		$query = new sql_query ($sql);
		return $this -> get_list ($query);
	}
	 ...
	 *
	 * @param String $id utam_wishlist primary key
	 */
	public function delete ($id)
	{
		$sql = 'DELETE FROM utam_wishlist WHERE id = ?';
		$query = new sql_query ($sql);
		$query -> set_number ($id);
	...
	{
		$sql = 'SELECT * FROM utam_wishlist WHERE id = ?';
		$query = new sql_query ($sql);
		$query -> set ($value);
		return $this -> get_list ($query);
	}
	...
	{
		$sql = 'SELECT * FROM utam_wishlist WHERE isbn = ?';
		$query = new sql_query ($sql);
		$query -> set ($value);
		return $this -> get_list ($query);
	}
	...
	{
		$sql = 'DELETE FROM utam_wishlist WHERE isbn = ?';
		$query = new sql_query ($sql);
		$query -> set ($value);
		return $this -> execute_update ($query);
	}
	 ...
	 *
	 * @param utam_wishlist_mysql utam_wishlist.
	 */
	public function insert ($utam_wishlist)
	{
		$sql = 'INSERT INTO utam_wishlist (id, isbn) VALUES (?, ?)';
		$query = new sql_query ($sql);

		$query -> set ($utam_wishlist -> id);
		$query -> set ($utam_wishlist -> isbn);
	 ...
	 * @param utam_wishlist_mysql utam_wishlist
	 */
	public function update ($utam_wishlist)
	{
		$sql = 'UPDATE utam_wishlist SET id = ?, isbn = ? WHERE id = ?';
		$query = new sql_query ($sql);

		$query -> set ($utam_wishlist -> id);
		$query -> set ($utam_wishlist -> isbn);

		$query -> set_number ($utam_wishlist -> id);
	...
	{
		$utam_wishlist = new utam_wishlist ();

		$utam_wishlist -> id = $row['id'];
		$utam_wishlist -> isbn = $row['isbn'];

		return $utam_wishlist;
	}
